...
const FeaturedSkeleton = () => (
  <div style={{
    background: '#1c1812', padding: 'clamp(36px, 6vw, 64px)',
    display: 'flex', flexDirection: 'column', gap: 16,
  }}>
    <Skeleton width={40} height={2} style={{ background: '#b8955a', animation: 'none' }} />
    <Skeleton width={80} height={22} style={{ background: 'rgba(255,255,255,0.08)', animation: 'none' }} />
    <Skeleton width="80%" height={36} style={{ background: 'rgba(255,255,255,0.06)', animation: 'none' }} />
    <Skeleton width="60%" height={36} style={{ background: 'rgba(255,255,255,0.06)', animation: 'none' }} />
    <Skeleton width="90%" height={16} style={{ marginTop: 8, background: 'rgba(255,255,255,0.04)', animation: 'none' }} />
    <Skeleton width="75%" height={16} style={{ background: 'rgba(255,255,255,0.04)', animation: 'none' }} />
  </div>
)

const PostRowSkeleton = () => (
  <div style={{ borderTop: '1px solid #ede5d8', padding: '28px 0', display: 'flex', gap: 20 }}>
    ...
    <>
      ...
      <main>
        ...
        <section style={{ maxWidth: 760, margin: '0 auto', padding: '80px 32px' }}>
          <div className="fade-up" ref={reveal} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
          ...
          {/* Loading skeleton */}
          {loading && !error && <FeaturedSkeleton />}
          ...
          {!loading && !error && featuredPost && (
            <article
              ...
              style={{ position: 'relative', background: '#1c1812', padding: 'clamp(36px, 6vw, 64px)', cursor: 'pointer', overflow: 'hidden' }}
              onMouseEnter={(e) => {
            ...
            >
              <span className="serif" style={{ position: 'absolute', top: -20, right: 32, fontSize: 200, lineHeight: 1, color: 'rgba(184,149,90,0.06)', fontStyle: 'italic', pointerEvents: 'none', userSelect: 'none' }}>"</span>
              <div style={{ width: 40, height: 2, background: '#b8955a', marginBottom: 28 }} />
              <Tag label={featuredPost.tag} />
              <h2 className="serif" style={{ fontSize: 'clamp(1.6rem, 3.8vw, 2.5rem)', fontWeight: 600, lineHeight: 1.2, color: '#fdfaf5', marginTop: 20, marginBottom: 18, maxWidth: 540 }}>
                {featuredPost.title}
              </h2>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <span style={{ fontSize: 12, color: 'rgba(253,250,245,0.4)', letterSpacing: '0.04em' }}>{featuredPost.date}</span>
                  <span style={{ color: 'rgba(253,250,245,0.2)' }}>·</span>
                  <span style={{ fontSize: 12, color: 'rgba(253,250,245,0.4)' }}>{featuredPost.read_time}</span>
                </div>
          ...
          {!loading && !error && !featuredPost && (
            <div style={{ padding: '48px 32px', textAlign: 'center', border: '1px dashed #ddd5c5' }}>
              <p style={{ fontSize: 14, color: 'rgba(28,24,18,0.4)', fontStyle: 'italic' }}>No posts yet. Add your first post in Supabase.</p>
            </div>
          )}
        </section>
        ...
        <section id="posts" style={{ maxWidth: 760, margin: '0 auto', padding: '0 32px 80px' }}>
          <div className="fade-up" ref={reveal} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
            <span style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(28,24,18,0.35)', fontWeight: 600 }}>
              Recent Writing
            </span>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {topics.map((t) => (
                <button key={t} className={`topic-pill ${activeTag === t ? 'active' : ''}`} onClick={() => setActiveTag(t)}>
                  {t}
                </button>
              ))}
            </div>
          ...
          {/* Loading skeletons */}
          {loading && !error && [1, 2, 3].map((n) => <PostRowSkeleton key={n} />)}
                ...
                filtered.map((post, i) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20 }}>
                      <span className="serif" style={{ fontSize: 11, color: 'rgba(28,24,18,0.2)', fontStyle: 'italic', paddingTop: 3, minWidth: 20, textAlign: 'right' }}>
                        {String(i + 1).padStart(2, '0')}
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                          <Tag label={post.tag} />
                          <span style={{ fontSize: 11, color: 'rgba(28,24,18,0.35)', letterSpacing: '0.03em' }}>{post.date}</span>
                          <span style={{ color: 'rgba(28,24,18,0.2)' }}>·</span>
                          <span style={{ fontSize: 11, color: 'rgba(28,24,18,0.35)' }}>{post.read_time}</span>
                        </div>
                        <h3 className="serif" style={{ fontSize: 'clamp(1.05rem, 2.6vw, 1.3rem)', fontWeight: 600, lineHeight: 1.35, color: '#1c1812', marginBottom: 8 }}>
                          {post.title}
                        </h3>
                        <p style={{ fontSize: 13, lineHeight: 1.7, color: 'rgba(28,24,18,0.5)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {post.excerpt}
                        </p>
                      </div>
          ...
          {!loading && filtered.length > 0 && (
            <div className="fade-up" ref={reveal} style={{ marginTop: 48, display: 'flex', justifyContent: 'center' }}>
              <a href="#" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', fontWeight: 600, color: '#1c1812', textDecoration: 'none', padding: '13px 36px', border: '1px solid #ccc4b4', transition: 'all 0.2s' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = '#1c1812'; e.currentTarget.style.color = '#fdfaf5'; e.currentTarget.style.borderColor = '#1c1812' }}
                onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#1c1812'; e.currentTarget.style.borderColor = '#ccc4b4' }}
              >
                View all posts
                <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
